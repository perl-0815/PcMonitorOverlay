...
配布された `dist-portable` フォルダを使う場合は、次の exe を実行してください。

```text
PcMonitorOverlayInstaller.exe
```

インストーラーは `PcMonitorOverlay.exe` をユーザー領域へコピーし、スタートメニューとデスクトップにショートカットを作成します。

ソースコードからインストールする場合は、次の PowerShell スクリプトを使えます。
...
- `PcMonitorOverlayInstaller.exe` と `PcMonitorOverlayUninstaller.exe` を作成する
...
インストール済みのアプリを削除する場合は、スタートメニューの `Uninstall PC Monitor Overlay`、または配布フォルダ内の次の exe を実行してください。

```text
PcMonitorOverlayUninstaller.exe
```

ソースコードから PowerShell スクリプトで削除する場合:
...
& $dotnet run --project .\src\PcMonitorOverlay\PcMonitorOverlay.csproj
```
...
アプリ本体、インストーラー、アンインストーラーを含む配布用フォルダを作成します。
...
出力先は既定で `dist-portable` です。`dist`、`dist-portable`、`bin`、`obj` は生成物なので Git 管理外にしています。
...
|   |-- PcMonitorOverlay.Installer/
|   |   `-- PcMonitorOverlay.Installer.csproj
|   `-- PcMonitorOverlay.Uninstaller/
|       `-- PcMonitorOverlay.Uninstaller.csproj
|-- scripts/
|   |-- build.ps1
|   |-- install.ps1
